...
"""
    Components' information example
    ===============================
...
from wirepas_messaging.wnt.ws_api import RealtimeSituationMessages, ComponentsInformationMessages


class Messages(RealtimeSituationMessages, ComponentsInformationMessages):
    ...
    def __init__(self, logger, protocol_version) -> None:
        ...
        ComponentsInformationMessages.__init__(self, logger, protocol_version)
...
class ComponentsInformationExample(object):
    ...
    def __init__(self) -> None:
        ...
        self.logger = wnt.utils.setup_log(
            'ComponentsInformationExample', self.settings.log_level)
    ...
    def parse_response(self, message: str) -> bool:
        ...
        if self.state == self.State.LOGIN:
            ...
            return self.messages.parse_query_components_information(json.loads(message))
    ...
    def realtime_situation_on_message(self, _websocket, message: str) -> None:
        ...
        if self.state == self.State.REALTIME_SITUATION_LOGIN:
            ...
            if wnt_message.HasField('backend_component_info'):
                self.logger.info(
                    'backend_component_info:\n{}'.format(wnt_message.backend_component_info))
